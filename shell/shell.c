...
int pathcmd(void)
{
	if (cmdPch[1] != NULL && strcmp(cmdPch[1], "+") == 0) {
		...
		if (current == NULL) {
			...
			strncpy(current->pathChar, cmdPch[2], SLEN-1);
		...
		if (strcmp(current->pathChar, cmdPch[2]) == 0) {
			temp = current;
			head = current->next;
			isInList = TRUE;
			/*If delete head.*/
...
int fsearch(void)
{
	...
	if (fileFound == FALSE)
		printf("ERROR: File not found in saved path.\n");
	return fileFound;
}
...
int execcmd(void)
{
	...
	if (pid < 0)
		...
		execl(filePath, cmdPch[0], cmdPch[1], NULL);
	else
